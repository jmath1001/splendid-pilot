...
import { Plus, Trash2, GraduationCap, Loader2, Save, X, Search } from 'lucide-react';
...
function StudentRow({ student, onRefetch }: { student: any; onRefetch: () => void }) {
  ...
  const handleUpdate = async () => {
    setSaving(true);
    const { error } = await supabase.from('students').update({ name }).eq('id', student.id);
    ...
    <div className="bg-white border border-stone-200 rounded-xl transition-all hover:shadow-sm overflow-hidden">
      <div className="p-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4 flex-1">
          ...
          {isEditing ? (
            <input value={name} onChange={e => setName(e.target.value)}
              className="px-3 py-1.5 border rounded-lg text-sm focus:ring-2 focus:ring-violet-500 outline-none flex-1" placeholder="Name" />
          ) : (
        ...
        <div className="flex items-center gap-2 self-end md:self-center">
          {isEditing ? (
            <>
              <button onClick={() => { setIsEditing(false); setName(student.name); }} className="p-2 text-stone-400 hover:text-stone-600"><X size={18} /></button>
            ...
            <>
              <button onClick={() => setIsEditing(true)} className="px-3 py-1.5 text-xs font-semibold text-stone-600 border border-stone-200 rounded-lg hover:bg-stone-50">Edit</button>
  ...
  return (
    <div className="min-h-screen bg-[#F9F8F6] text-stone-900 font-sans pb-20">
      ...
      <main className="max-w-4xl mx-auto px-6 pt-8 space-y-6">
        ...
        {adding && (
          <div className="bg-white border-2 border-violet-500 rounded-2xl p-6 shadow-xl shadow-violet-100">
            <div className="flex justify-between mb-4">
              <h2 className="font-bold text-lg text-violet-900">New Student</h2>
              <button onClick={() => setAdding(false)} className="text-stone-400 hover:text-stone-600"><X size={20} /></button>
            </div>
            <form onSubmit={handleCreate} className="flex gap-4">
              <input required value={newName} onChange={e => setNewName(e.target.value)}
                className="flex-1 px-4 py-2.5 bg-stone-50 border border-stone-200 rounded-xl focus:bg-white outline-none" placeholder="Student name" />
              <button type="submit" className="bg-violet-600 text-white px-6 py-2.5 rounded-xl font-bold hover:bg-violet-700 transition-colors">
                Add
              </button>
            </form>
          </div>
